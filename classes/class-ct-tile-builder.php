<?php
/** 
 * CT_Tile_Builder class
 *
 * The core class that generates the functionalities for the
 * Tile Builder. Almost nothing inside in the class should
...
if(!class_exists('CT_Tile_Builder')) {
	class CT_Tile_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = __('Tile Builder', 'framework');
			$defaults['page_title'] = __('Tile Builder', 'framework');
			$defaults['page_slug'] = __('ct-tile-builder', 'framework');
		...
		function init() {
			...
			add_action('init', array(&$this, 'register_template_post_type'));
                        add_action('init', array(&$this, 'register_tile_post_type'));
			...
			add_action('admin_footer', array(&$this, 'add_media_display') );
				...
				register_post_type( 'cttile', $template_args);
				add_action( 'admin_head-edit.php', array(&$this,'post_type_icon'));
                                add_action( 'admin_head-post-new.php', array(&$this,'post_type_icon'));
			} else {
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Tile Builder notice: </strong>'. __('The \"tile\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to consult with this issue', 'framework') .'</p></div>';"));
			}
                }
                ...
                function post_type_icon() {
                        ...
                        ?>
                        <style>
                        <?php if($post_type == 'cttile'){ ?>
                        .icon32-posts-<?php echo $post_type;?> {
                                background: url('<?php echo CTTB_DIR . '/assets/images/aqua-media-button.png';?>') no-repeat; }
                        <?php } ?>
                        </style>
                        <?php
                }
		...
		function admin_pages() {
			...
			$this->page = add_object_page( $this->args['page_title'], $this->args['menu_title'], 'manage_options', $this->args['page_slug'], array(&$this, 'builder_page_show'));
			add_submenu_page($this->args['page_slug'], $this->args['page_title'], 'Tile Content', 'manage_options', 'edit.php?post_type=cttile');
			add_submenu_page($this->args['page_slug'], $this->args['page_title'], 'Add New Content', 'manage_options', 'post-new.php?post_type=cttile');
		...
		function add_admin_bar(){
			global $wp_admin_bar;
			$wp_admin_bar->add_menu( array( 'id' => 'aq-page-builder', 'parent' => 'appearance', 'title' => 'Tile Builder', 'href' => admin_url('admin.php?page='.$this->args['page_slug']) ) );
		...
		function admin_enqueue() {
			...
			// Register 'em
			wp_register_style( 'aqpb-css', $this->url.'assets/css/cttb.css', array(), time(), 'all');
			wp_register_style( 'aqpb-blocks-css', $this->url.'assets/css/cttb_blocks.css', array(), time(), 'all');
			wp_register_script('aqpb-js', $this->url . 'assets/js/cttb.js', array('jquery'), time(), true);
			wp_register_script('cttb-fields-js', $this->url . 'assets/js/cttb-fields.js', array('jquery'), time(), true);
			
                        wp_register_style( 'ctsck-metrojs-css', $this->url.'assets/js/metrojs/metrojs.css', array(), time(), 'all');
			wp_register_script('ctsck-metrojs-js', $this->url . 'assets/js/metrojs/metrojs.js', array('jquery'), time(), true);
			...
			wp_enqueue_script('cttb-fields-js');
			
                        wp_enqueue_style('ctsck-metrojs-css');
                        wp_enqueue_script('ctsck-metrojs-js');
		...
		function view_enqueue() {
			...
			// front-end css
			wp_register_style( 'aqpb-view-css', $this->url.'assets/css/cttb-view.css', array(), time(), 'all');
			wp_enqueue_style('aqpb-view-css');
				
			// front-end js
			wp_register_script('aqpb-view-js', $this->url . 'assets/js/cttb-view.js', array('jquery'), time(), true);
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('cttemplate')) {
				...
				register_post_type( 'cttemplate', $template_args);
				
			} else {
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Tile Builder notice: </strong>'. __('The \"cttemplate\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to consult with this issue', 'framework') .'</p></div>';"));
			}
		...
		function is_template($template_id) {
			...
			if($template) {
				if($template->post_type != 'cttemplate' || $template->post_status != 'publish') return false;
			} else {
				return false;
			}
		...
		**/
		...
		function preview_template() {
			...
			if($post_type == 'cttemplate') {
				get_header();
				?>
					<div id="main" class="cf">
						<div id="content" class="cf">
							<?php $this->display_template(get_the_ID()); ?>
							<?php if($this->args['debug'] == true) print_r(ct_get_blocks(get_the_ID())) //for debugging ?>
						</div>
					</div>
				<?php
				get_footer();
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<style type="text/css">
                                ...
                                echo '<section class="tile-group" id="tile-group-'.$template_id.'">';
		...
		function add_shortcode() {
			...
			if ( !array_key_exists( 'tile-template', $shortcode_tags ) ) {
				add_shortcode( 'tile-template', array(&$this, 'do_shortcode') );
			} else {
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Tile Builder notice: </strong>'. __('The \"[tile-template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', 'framework') .'</p></div>';"));
			}
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . CTTB_DIR . '/assets/images/aqua-media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Tile Template', 'framework' )  . '" />';
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox" title="' . esc_attr__( 'Add Tile Template', 'framework' )  . '">' . $img . '</a>';
				} else {
					$img 	= '<span class="wp-media-buttons-icon" style="background-image: url(' . CTTB_DIR . '/assets/images/aqua-media-button.png ); margin-top: -1px;"></span>';
					$output = '<a href="#TB_inline?width=640&inlineId=aqpb-iframe-container" class="thickbox button" title="' . esc_attr__( 'Add Tile Template', 'framework' ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', 'framework' ) . '</a>';
				}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<script type="text/javascript">
					function insertTemplate() {
						...
						window.send_to_editor('[tile-template id="' + id + '"]');
					}
				</script>
				...
				<div id="aqpb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">
						...
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', 'framework' ), '<a href="' .admin_url().'admin.php?page=aq-page-builder">', '</a>' );
							return;
						}
						?>						

						<h3><?php _e( 'Choose Your Tile Template', 'framework' ); ?></h3><br />
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					...
					'<p>' . __('<a href="http://crosstec.de/" target="_blank">Documentation</a>') . '</p>' .
					'<p>' . __('<a href="http://crosstec.de/" target="_blank">Changelog</a>') . '</p>'
				);
		...
		/**
		 * Main tile builder page display
		 *
		 * @since	1.0.0
		 */
		function builder_page_show(){
		
			require_once(CTTB_PATH . 'view/view-builder-page.php');
